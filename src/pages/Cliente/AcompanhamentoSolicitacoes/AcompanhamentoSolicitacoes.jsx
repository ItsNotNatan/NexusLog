...
export default function AcompanhamentoSolicitacoes({ perfil = 'cliente' }) {
  ...
  // 👇 MUDANÇA AQUI: Agora recebe o idUnico em vez do ID normal
            ...
            <table className="tabela-solicitacoes">
              ...
              <tbody>
                {dadosFiltrados.map((linha, index) => {
                  ...
                  // 👇 MUDANÇA AQUI: Criamos um ID 100% único juntando prefixo e número
                  const idUnico = `${linha.prefixo}-${linha.id}-${index}`;
                  const isExpandida = linhaExpandida === idUnico;
                  
                  return (
                    // Usamos o idUnico também como key do React
