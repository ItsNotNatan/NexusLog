...
export default function DetalhesSolicitacao({ item }) {
  ...
  if (item.tipo === 'Material') {
    // MOCK: Dados fiéis à tua imagem para vermos o layout funcionar
    const itensMaterial = [
  ...
  if (item.tipo === 'Nota Fiscal') {
    const descricao = item.observacoes || 'TESTE';
    const valorEstimado = item.valorEstimado || 'R$ 150000.00';

    return (
      <div className="area-expandida-cliente" style={{ padding: '16px 24px' }}>
        <div style={{ display: 'flex', alignItems: 'center', gap: '32px', fontSize: '0.875rem' }}>
          <div>
            <span style={{ color: '#1e293b', fontWeight: '600', marginRight: '6px' }}>Descrição:</span>
