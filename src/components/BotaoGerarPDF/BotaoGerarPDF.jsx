...
// ✨ AGORA RECEBE TAMBÉM AS NOVAS FUNÇÕES
export default function BotaoGerarPDF({ linha, nomeFilial, showAlert, showLoading, closeAlert }) {
  ...
  const handleGerarPdf = async (e) => {
    ...
    // ✨ INVOCA A NOVA TELA DE CARREGAMENTO (Sem Botão OK)
    if (showLoading) {
    ...
    try {
      ...
      const docDefinition = {
        ...
        content: [
          logoBase64 ? { image: logoBase64, width: 85, alignment: 'center', margin: [0, 0, 0, 8] } : { text: 'STOCKLog', style: 'headerMain', color: '#2563eb', margin: [0, 0, 0, 5] },
          ...
          { table: { widths: ['*'], body: [[{ text: 'INFORMAÇÕES DA SOLICITAÇÃO', style: 'sectionTitle', fillColor: '#f1f5f9' }]] }, layout: 'noBorders', margin: [0, 0, 0, 4] },
          {
            table: {
              widths: [150, '*'],
              body: [
                ...
                ...(linha.nfCrossdocking ? [ [{ text: 'NF Vinculada (Crossdocking):', bold: true, margin: [0, 4], color: '#b45309' }, { text: linha.nfCrossdocking, margin: [0, 4], bold: true, color: '#d97706' }] ] : [])
              ]
            },
            layout: 'lightHorizontalLines',
            margin: [0, 0, 0, 20]
          },
          { table: { widths: ['*'], body: [[{ text: isReintegracao ? `ITENS DEVOLVIDOS AO ESTOQUE` : `DETALHAMENTO DE ITENS DA SOLICITAÇÃO`, style: 'sectionTitle', fillColor: '#f1f5f9' }]] }, layout: 'noBorders', margin: [0, 0, 0, 4] },
          {
            table: {
          ...
          linha.observacoes ? { table: { widths: ['*'], body: [[{ text: 'OBSERVAÇÕES / JUSTIFICATIVA', style: 'sectionTitle', fillColor: '#fefce8', color: '#b45309' }]] }, layout: 'noBorders', margin: [0, 0, 0, 4] } : {},
          linha.observacoes ? { table: { widths: ['*'], body: [[{ text: linha.observacoes, margin: [5, 4, 5, 4], color: '#78350f' }]] }, margin: [0, 0, 0, 20] } : {},
          {
            margin: [0, 60, 0, 0], 
            columns: [
              { width: '*', stack: [ { canvas: [{ type: 'line', x1: 0, y1: 0, x2: 200, y2: 0, lineWidth: 1 }] }, { text: isReintegracao ? 'Entregue por (Solicitante)' : 'Solicitante', style: 'signatureLabel', margin: [0, 5, 0, 2] }, { text: linha.solicitante, fontSize: 9, color: '#64748b' } ], alignment: 'center' },
              { width: '*', stack: [ { canvas: [{ type: 'line', x1: 0, y1: 0, x2: 200, y2: 0, lineWidth: 1 }] }, { text: isReintegracao ? 'Recebido por (Almoxarifado)' : 'Responsável Almoxarifado / Logística', style: 'signatureLabel', margin: [0, 5, 0, 2] }, { text: 'Data e Assinatura', fontSize: 9, color: '#64748b' } ], alignment: 'center' }
            ]
          },
          { text: `Documento gerado automaticamente pelo sistema STOCKLog em ${new Date().toLocaleString('pt-BR')}`, alignment: 'center', fontSize: 8, color: '#94a3b8', margin: [0, 40, 0, 0] }
        ],
        styles: {
        ...
        defaultStyle: { fontSize: 9.5, color: '#334155', font: 'Roboto' }
      };
      ...
      // ✨ OBTÉM SUCESSO: Removemos a animação e fechamos o modal sem pedir que o utilizador clique!
      setGerando(false);
      if (closeAlert) {
        closeAlert();
      }
  ...
  return (
    <span 
      ...
      style={{ cursor: gerando ? 'not-allowed' : 'pointer', transition: 'all 0.2s', userSelect: 'none', opacity: gerando ? 0.6 : 1, display: 'inline-flex', alignItems: 'center', gap: '4px' }}
      title="Clique para abrir o Boletim Detalhado em PDF numa nova aba"
      onMouseOver={(e) => { if(!gerando) { e.currentTarget.style.backgroundColor = '#dbeafe'; e.currentTarget.style.borderColor = '#93c5fd'; } }}
      onMouseOut={(e) => { if(!gerando) { e.currentTarget.style.backgroundColor = '#eff6ff'; e.currentTarget.style.borderColor = '#bfdbfe'; } }}
    >
      <FileText size={14} /> 
